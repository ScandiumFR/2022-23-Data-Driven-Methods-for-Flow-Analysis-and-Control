...
phi1 = C*1/sqrtQ*V(:,i); % response mode
psi1 = C*1/sqrtQ*U(:,i); % forcing mode
length = N-2;
...
subplot(1,3,1)
title('Phi U')
plot(y(2:end-1),phi1(1:length).*conj(phi1(1:length)),'LineWidth',2) %removed 'b' for line colour
hold on
subplot(1,3,2)
title('Phi V')
plot(y(2:end-1),phi1(length+1:2*length).*conj(phi1(length+1:2*length)),'r--','LineWidth',2)
hold on
subplot(1,3,3)
title('Phi W')
plot(y(2:end-1),phi1(2*length+1:3*length).*conj(phi1(2*length+1:3*length)),'k-.','LineWidth',2)
hold on
